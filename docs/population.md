...
Synthesis population generated by Synpop output modes for trips (walk, bike, car, car-passenger,pt).
Modes are set according to activites chains data in ENTD/EMD as input and might not match
local offer (i.e : no public transport available, too long distances for walk).
This default behavior is fixed in the process if you uses MATsim for trafic simulation as next step.
In case you need full mode choice model (accurate modes results) and synthesis population is your final step
in the process (no MATsim simulation interest) you can set in the config.yml file : 

```bash
mode_choice=True
```

Doing so a single MATsim trafic simulation will run and output modes according to its mode choice model.
As a simulation is required you will need to provide 2 more data sources according to [this procedure](https://github.com/eqasim-org/ile-de-france/blob/develop/docs/simulation.md) :
