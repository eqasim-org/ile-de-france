...
FIELDS = ["person_id", "trip_index", "preceding_purpose", "following_purpose", "distance"]
FIXED_PURPOSES = ["home"]
...
def find_bare_assignment_problems(df):
    ...
    for row in df[FIELDS].itertuples(index = False):
        person_id, trip_index, preceding_purpose, following_purpose, distance = row
        ...
        problem["distances"].append(distance)
